...
const Login = () => {
  // Allows error to be displayed to the user
  const errRef = useRef();

  // Error message for user if the Username and Password do not match
  const [errMsg, setErrMsg] = useState('');

  // Error message for user when server is down
  ...
  const { addingItem } = useSelector((state) => state.auth);
  const title = 'Login';
  const description = 'Login Page';
  const history = useHistory();

  // Where the user will be redirected after successful login - if (adding an item)
  const addItemForm = {
    pathname: '/listMyItem',
    state: { from: 'login' },
  };
  ...
  const onSubmit = (values) => {
    ...
    if (!serverConnect) {
      setErrMsgServer('The server is down, please try agian later.');
    }
    ...
    if (!authenticate) {
      ...
      setErrMsg('Email and password did not match a registered user.');
    }
  };
    ...
    <div className="sw-lg-70 min-h-100 bg-foreground d-flex justify-content-center align-items-center shadow-deep py-5 full-page-content-right-border">
      <div className="sw-lg-50 px-5">
        ...
        <div className="mb-5">
          ...
          <p style={{ color: 'red' }} ref={errRef} className={errMsg ? 'errmsgEmail' : 'offscreen'}>
            {errMsg}
          </p>
          <p style={{ color: 'red' }} ref={errRef} className={errMsgServer ? 'errmsgEmail' : 'offscreen'}>
            {errMsgServer}
          </p>
        </div>
        <div>
