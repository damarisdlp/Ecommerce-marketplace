...
const MainMenu = () => {
  ...
  const menuItemsMemoLI = useMemo(
    () =>
      getMenuItems({
        data: attrMobile && useSidebar ? routesAndMenuItemsLI : routesAndMenuItemsLI.mainMenuItems,
        isLogin,
        userRole: currentUser.role,
      }),
    [isLogin, currentUser, attrMobile, useSidebar]
    ...
    if (placementStatus.view === MENU_PLACEMENT.Horizontal) {
      ...
      if (isLogin) {
        return (
          <div className="menu-container flex-grow-1">
            <ul id="menu" className={classNames('menu show')}>
              <MainMenuItems menuItems={menuItemsMemoLI} menuPlacement={placementStatus.view} />
            </ul>
          </div>
        );
      }
